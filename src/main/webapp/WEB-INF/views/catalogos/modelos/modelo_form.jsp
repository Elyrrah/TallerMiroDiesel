...
<form method="post" action="${pageContext.request.contextPath}/modelos">
    ...
    <a href="${pageContext.request.contextPath}/modelos?action=listar<c:if test='${not empty param.idMarca}'>&idMarca=${param.idMarca}</c:if><c:if test='${not empty param.filtro}'>&filtro=${param.filtro}</c:if>">
        Cancelar
    </a>
</form>
