...
-- trigger clientes: exige al menos distrito o localidad; si viene localidad setea distrito; si vienen ambos valida coherencia
CREATE OR REPLACE FUNCTION public.fn_clientes_setear_validar_ubicacion()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    -- si viene localidad, obtenemos su distrito real
    IF NEW.id_localidad IS NOT NULL THEN
        ...
        IF v_id_distrito IS NULL THEN
            RAISE EXCEPTION 'LOCALIDAD % NO EXISTE.', NEW.id_localidad;
        END IF;

        -- si no vino distrito, autocompletar
        IF NEW.id_distrito IS NULL THEN
            NEW.id_distrito := v_id_distrito;
        ELSE
            -- si vino distrito, validar coherencia
            IF NEW.id_distrito <> v_id_distrito THEN
                RAISE EXCEPTION 'LA LOCALIDAD % NO PERTENECE AL DISTRITO %. DISTRITO CORRECTO: %.',
                    NEW.id_localidad, NEW.id_distrito, v_id_distrito;
            END IF;
        END IF;
    END IF;
...
DROP TRIGGER IF EXISTS trg_clientes_validar_ubicacion ON public.clientes;

CREATE TRIGGER trg_clientes_validar_ubicacion
BEFORE INSERT OR UPDATE ON public.clientes
FOR EACH ROW
EXECUTE FUNCTION public.fn_clientes_setear_validar_ubicacion();
...
-- ORDENES_TRABAJO: CABECERA DE ÓRDENES
...
-- ORDEN_TRABAJO_DETALLES: DETALLES DE SERVICIOS APLICADOS
CREATE TABLE IF NOT EXISTS public.orden_trabajo_detalles (
    ...
    -- detalles de orden: impide precio unitario negativo
...
-- PLANES_PAGO: ACUERDOS DE PAGO DE UNA ORDEN
...
-- PLANES_PAGO_EVENTOS: EVENTOS DEL PLAN (PAGOS, VENCIMIENTOS, ETC.)
