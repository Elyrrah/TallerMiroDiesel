...
CREATE TABLE IF NOT EXISTS public.tipos_documento (
    ...
    activo boolean DEFAULT true NOT NULL,
    CONSTRAINT tipos_documento_aplica_a_check
        CHECK (aplica_a IN ('PERSONA', 'EMPRESA', 'AMBOS'))
);
...
CREATE TABLE IF NOT EXISTS public.clientes (
    ...
    CONSTRAINT clientes_id_usuario_creador_fkey
        FOREIGN KEY (id_usuario_creador) REFERENCES public.usuarios(id_usuario)
);
...
CREATE TABLE IF NOT EXISTS public.ordenes_trabajo (
    ...
    -- cantidad de picos asociados a la OT (aplica cuando el componente es bomba o juego de picos)
    cantidad_picos smallint,
    -- referencia: cómo llegó el cliente al taller en esta visita
    fuente_referencia public.fuente_referencia_enum DEFAULT 'NINGUNA' NOT NULL,
    id_referidor bigint,
    ...
    estado_pago public.estado_pago_enum DEFAULT 'PENDIENTE' NOT NULL,
    ...
    CONSTRAINT ordenes_trabajo_id_referidor_fkey
        FOREIGN KEY (id_referidor) REFERENCES public.clientes(id_cliente),
    ...
    -- si hay referidor, la fuente no puede ser NINGUNA (y viceversa)
    CONSTRAINT ordenes_trabajo_ref_fuente_y_referidor
        CHECK (
            (fuente_referencia = 'NINGUNA' AND id_referidor IS NULL)
            OR
            (fuente_referencia <> 'NINGUNA' AND id_referidor IS NOT NULL)
        ),
    -- cantidad de picos: si se informa debe ser entre 1 y 12
    CONSTRAINT ordenes_trabajo_cantidad_picos_rango
        CHECK (cantidad_picos IS NULL OR (cantidad_picos >= 1 AND cantidad_picos <= 12)),
...
DECLARE
    ...
    nuevo_estado_pago public.estado_pago_enum;
BEGIN
